...
function shangxiaweiyi:seek(time)
	if self.first then
		if self.text ~= nil then
			...
			self.sharedMaterial = InsMaterials
			materials:pushBack(self.sharedMaterial)
			self.renderer.sharedMaterials = materials
			self.sharedMaterial:setInt('u_ANIMSEQ', 0)
		else
			self.sharedMaterial:setInt('u_ANIMSEQ', 1)
		end
		self.first = false
	end
	...
	playAnimation(info, time / self.duration, function (key, value)
		...
		elseif key == 'blurType' then
            self.sharedMaterial:setInt('u_BLUR_TYPE', value)
...
function shangxiaweiyi:reSetData()
...
function shangxiaweiyi:onEnter()
	self.clearState = true
	self.first = true
end

function shangxiaweiyi:onLeave()
	self:ResetData()
	self.first = true
